...
class TestRealLLMIntegration:
    ...
    @pytest.mark.asyncio
    async def test_agent_simple_task(self, llm, sandbox, tool_registry, session, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_agent_with_artifacts(self, llm, sandbox, tool_registry, session, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_agent_timeout(self, llm, sandbox, tool_registry, session, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_agent_delegation_with_artifacts(self, llm, sandbox, tool_registry, session, agent_config):
        ...
        if result.success:
            assert "titanic" in result.output.lower() or "script" in result.output.lower()
    ...
    @pytest.mark.asyncio
    async def test_agent_with_console_tool(self, llm, sandbox, tool_registry, session, agent_config):
    ...
    @pytest.mark.asyncio
    async def test_agent_with_files_tool(self, llm, sandbox, tool_registry, session, agent_config):
...
3,1,3,"Heikkinen, Miss. Laina",female,26,0,0,STON/O2. 3101282,7.925,,S"""
        ...
        result = await agent.run(
            "Read train.csv and tell me how many passengers survived",
            context={"session": session},
            timeout=120
        )
        ...
        assert "2" in result.output or "survived" in result.output.lower()
    
    @pytest.mark.asyncio
    async def test_titanic_artifact_workflow(self, llm, sandbox, tool_registry, session):
...
2,1,1,"Cumings, Mrs. John Bradley (Florence Briggs Thayer)",female,38,1,0,PC 17599,71.2833,C85,C"""
        ...
        def agent_factory1(**kwargs):
            return BaseAgent(agent_config1, llm, sandbox, tool_registry)
        
        agent1 = agent_factory1()
        
        result1 = await agent1.run(
            "Read train.csv and store the number of survivors in an artifact called 'survivor_count'",
            context={"session": session},
            timeout=120
        )
        ...
        def agent_factory2(**kwargs):
            return BaseAgent(agent_config2, llm, sandbox, tool_registry)
        
        agent2 = agent_factory2()
        
        result2 = await agent2.run(
            "Read the survivor_count artifact and write a report to report.txt",
            context={"session": session, "artifacts": session.get_all_artifacts()},
            timeout=120
        )
