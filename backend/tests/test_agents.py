...
from kaggle_solver.tools import register_rag_tool
...
class TestCoordinatorAgent:
    ...
    def test_coordinator_has_delegate_tool(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
    ...
    def test_coordinator_delegates_task(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        ...
        sub_agent = Mock()
        sub_agent.run = AsyncMock(
            return_value=AgentResult(success=True, output="work done")
        )
    ...
    def test_coordinator_handles_subagent_failure_via_llm_decision(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """Coordinator should continue after delegated failure and decide next action itself."""
        config = AgentConfig(
            name="Coordinator",
            role="Plan and delegate",
            tools=["delegate", "message", "tool"],
        ...
        mock_llm.chat = AsyncMock(
            side_effect=[
                '{"action": "delegate", "agent": "CodeAgent", "task": "do work"}',
                '{"action": "error", "error": "worker failed"}',
            ]
        )

        sub_agent = Mock()
        sub_agent.run = AsyncMock(
            return_value=AgentResult(success=False, error="worker failed")
        )
        agent.agent_factory = Mock(return_value=sub_agent)
    ...
    def test_coordinator_handles_failed_plan_step_via_llm_decision(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """Coordinator should decide how to handle a failed plan step instead of stopping automatically."""
        config = AgentConfig(
            name="Coordinator",
            role="Plan and delegate",
            tools=["delegate", "message", "tool"],
        ...
        mock_llm.chat = AsyncMock(
            side_effect=[
                '{"action": "plan", "plan_id": "test123", "steps": [{"id": 1, "agent": "CodeAgent", "task": "do work"}]}',
                '{"action": "update_plan", "plan_id": "test123", "step_updates": [{"id": 1, "status": "blocked", "note": "worker failed"}]}',
                '{"action": "done", "result": "Blocked: worker failed"}',
            ]
        )
        ...
        result = asyncio.run(agent.run("execute a plan"))

        assert result.success is True
        assert result.output == "Blocked: worker failed"
    ...
    def test_coordinator_system_prompt(
        ...
        config = AgentConfig(
            name="Coordinator", role="test", tools=[], max_iterations=3
        )
        agent = create_agent(
            ConcreteCoordinator,
            config,
    ...
    def test_coordinator_uses_all_tools(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
...
class TestCodeAgent:
    ...
    def test_code_agent_has_console_tool(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        ...
        config = AgentConfig(
            name="CodeAgent",
            role="Execute code",
            tools=["console", "files", "pip_install"],
            max_iterations=3,
        )
        agent = create_agent(
            ConcreteCode,
    ...
    def test_code_agent_has_files_tool(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
    ...
    def test_code_agent_has_pip_install_tool(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """CodeAgent should have pip_install tool."""
        config = AgentConfig(
            name="CodeAgent",
            role="Execute code",
            tools=["console", "files", "pip_install"],
        ...
        assert "pip_install" in agent.config.tools

    def test_code_agent_executes_console(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        ...
        mock_llm.chat = AsyncMock(
            return_value='{"action": "tool", "tool": "console", "query": "echo hello"}'
        )
    ...
    def test_code_agent_executes_files(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        ...
        mock_llm.chat = AsyncMock(
            return_value='{"action": "tool", "tool": "files", "query": "test.py"}'
        )
        mock_tool_registry.execute.return_value = ToolResult(
    ...
    def test_code_agent_executes_pip_install(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """CodeAgent should execute pip_install."""
        config = AgentConfig(
            name="CodeAgent",
            role="Execute code",
            tools=["console", "files", "pip_install"],
        ...
        mock_llm.chat = AsyncMock(
            return_value='{"action": "tool", "tool": "pip_install", "packages": "pandas"}'
        )
        mock_tool_registry.execute.return_value = ToolResult(
            success=True, output="installed"
        )

        asyncio.run(agent.run("install pandas"))

        mock_tool_registry.execute.assert_called()
        call_args = mock_tool_registry.execute.call_args
        assert call_args[0][0] == "pip_install"
...
class TestSearchAgent:
    ...
    def test_search_agent_has_search_tool(
        ...
        config = AgentConfig(
            name="SearchAgent", role="Search web", tools=["search"], max_iterations=3
        )
        agent = create_agent(
            ConcreteSearch,
            config,
    ...
    def test_search_agent_executes_search(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        ...
        mock_llm.chat = AsyncMock(
            return_value='{"action": "tool", "tool": "search", "query": "python tutorial"}'
        )
        mock_tool_registry.execute.return_value = ToolResult(
    ...
    def test_search_agent_system_prompt(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """SearchAgent should have correct system prompt."""
        config = AgentConfig(
            name="SearchAgent", role="test", tools=[], max_iterations=3
        )
        agent = create_agent(
            ConcreteSearch,
            config,
        ...
        assert (
            "SearchAgent" in prompt
            or "search" in prompt.lower()
            or "web" in prompt.lower()
        )
...
class TestCriticAgent:
    ...
    def test_critic_agent_has_message_tool(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
    ...
    def test_critic_agent_can_validate(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        ...
        mock_llm.chat = AsyncMock(
            return_value='{"action": "done", "result": "VALID - solution looks good"}'
        )
    ...
    def test_critic_agent_system_prompt(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        config = AgentConfig(
            name="CriticAgent", role="test", tools=[], max_iterations=3
        )
        agent = create_agent(
            ConcreteCritic,
            config,
        ...
        assert (
            "qa" in prompt.lower()
            or "quality" in prompt.lower()
            or "critic" in prompt.lower()
        )
...
class TestAllToolsAvailable:
    ...
    def setup_method(self):
        ...
        ToolRegistry.register(
            "console", console_tool, "Execute shell commands in sandbox"
        )
        ToolRegistry.register("files", files_tool, "File operations in sandbox")
        ToolRegistry.register(
            "pip_install", pip_install_tool, "Install Python packages in the sandbox"
        )
        ToolRegistry.register("search", search_tool, "Web search via DuckDuckGo")
        register_rag_tool()
    ...
    def test_pip_install_tool_registered(self):
        """pip_install tool should be registered."""
        from kaggle_solver.tools.pip_install import pip_install_tool

        tool = ToolRegistry.get("pip_install")
        assert tool is not None
    ...
    def test_files_tool_operations(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Write
            result = files_tool(
                op="write", path="test.txt", content="hello", sandbox=sandbox
            )
...
class TestToolRegistry:
    ...
    def test_tool_with_sandbox(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = ToolRegistry.execute(
                "console", query="echo test123", sandbox=sandbox
            )
    ...
    def test_tool_with_llm(self):
        ...
        result = ToolRegistry.execute("search", query="test query", llm=Mock())
...
class TestAgentToolsIntegration:
    ...
    def test_coordinator_can_use_tool(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """Coordinator can use tool action."""
        config = AgentConfig(
            name="Coordinator", role="test", tools=["tool"], max_iterations=2
        )
        agent = create_agent(
            ConcreteCoordinator,
            config,
        ...
        mock_llm.chat = AsyncMock(
            return_value='{"action": "tool", "tool": "console", "query": "ls"}'
        )
    ...
    def test_search_agent_with_real_search(
        self, mock_llm, mock_sandbox, mock_tool_registry, mock_event_callback
    ):
        """SearchAgent can use real search tool."""
        config = AgentConfig(
            name="SearchAgent", role="test", tools=["search"], max_iterations=2
        )
        agent = create_agent(
            ConcreteSearch,
            config,
...
class TestAgentPrompts:
    ...
    def test_get_agent_prompts(self):
        ...
        assert len(get_agent_prompts("HypothesisGenerator")) > 0
        assert len(get_agent_prompts("DataPreprocessor")) > 0
        assert len(get_agent_prompts("FeatureEngineer")) > 0
        assert len(get_agent_prompts("ModelTrainer")) > 0
        assert len(get_agent_prompts("DataParser")) > 0
        assert len(get_agent_prompts("KaggleSubmitter")) > 0

    def test_python_workflow_prompts_include_pip_install(self):
        for agent_name in [
            "CodeAgent",
            "HypothesisGenerator",
            "DataPreprocessor",
            ...
            assert "pip_install" in get_agent_prompts(agent_name)


class TestAgentToolMappings:
    def test_python_workflow_agents_include_pip_install(self):
        for agent_name in [
            "CodeAgent",
            "HypothesisGenerator",
            "DataPreprocessor",
            ...
            assert "pip_install" in BaseAgent.get_tools_for_agent(agent_name)
