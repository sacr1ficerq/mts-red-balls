...
class TestKaggleTools:
    ...
    def mock_kaggle_client(self):
        ...
        with patch(
            "kaggle_solver.tools.kaggle.get_kaggle_mcp_client"
        ) as mock_get_client:
            mock_client = Mock()
    ...
    def sample_submission_df(self):
        ...
        return pd.DataFrame({"PassengerId": [892, 893, 894], "Survived": [0, 1, 0]})

    @pytest.fixture
    def sample_predictions_df(self):
        """Sample predictions DataFrame"""
        return pd.DataFrame(
            {"PassengerId": [892, 893, 894], "Survived": [0.2, 0.8, 0.3]}
        )

    def test_kaggle_get_competition_info_success(
        self, mock_kaggle_client, sample_competition_info
    ):
    ...
    def test_kaggle_get_competition_info_competition_alias(
        self, mock_kaggle_client, sample_competition_info
    ):
        """Accept `competition` alias used by some agent outputs."""
        mock_kaggle_client.get_competition_info.return_value = sample_competition_info

        result = kaggle_get_competition_info(competition="titanic")

        assert result["success"] is True
        assert result["data"] == sample_competition_info
        mock_kaggle_client.get_competition_info.assert_called_once_with("titanic")

    def test_kaggle_get_competition_info_missing_query(self):
        """Return a clean validation error if competition is missing."""
        result = kaggle_get_competition_info()
        assert result["success"] is False
        assert "Competition query is required" in result["error"]
    ...
    def test_kaggle_get_competition_info_no_client(self):
        ...
        with patch(
            ...
            assert result["success"] is False
            assert "Kaggle client not available" in result["error"]
    ...
    def test_kaggle_download_data_specific_files(self, mock_kaggle_client):
        ...
        result = kaggle_download_data(
            query="titanic", path="./data", files="train.csv,test.csv"
        )
    ...
    def test_kaggle_download_data_competition_alias(self, mock_kaggle_client):
        """Accept `competition` alias when query is not present."""
        mock_kaggle_client.download_competition_data.return_value = ["./data/train.csv"]

        result = kaggle_download_data(competition="titanic", path="./data")
    ...
    def test_kaggle_submit_success(self, mock_kaggle_client):
        ...
        result = kaggle_submit(
            query="titanic", submission_file="submission.csv", message="Test submission"
        )
    ...
    def test_kaggle_get_submission_status_success(self, mock_kaggle_client):
        ...
        result = kaggle_get_submission_status(query="titanic", submission_id="12345678")

        assert result["success"] is True
        assert result["data"]["public_score"] == 0.78947
        mock_kaggle_client.get_submission_status.assert_called_once_with(
            "titanic", "12345678"
        )
    ...
    def test_kaggle_list_competitions_success(self, mock_kaggle_client):
        ...
        mock_kaggle_client.list_competitions.assert_called_once_with(
            category="gettingStarted", search=""
        )
    ...
    def test_kaggle_validate_submission_invalid_columns(
        self, sample_submission_df, tmp_path
    ):
        ...
        invalid_df = pd.DataFrame({"ID": [892, 893, 894], "Target": [0, 1, 0]})
    ...
    def test_kaggle_validate_submission_missing_values(
        self, sample_submission_df, tmp_path
    ):
    ...
    def test_kaggle_prepare_submission_success(
        self, sample_submission_df, sample_predictions_df, tmp_path
    ):
    ...
    def test_kaggle_prepare_submission_sorts_by_id(
        self, sample_submission_df, tmp_path
    ):
        ...
        unsorted_df = pd.DataFrame(
            {"PassengerId": [894, 892, 893], "Survived": [0.3, 0.2, 0.8]}
        )
...
class TestKaggleToolsIntegration:
    ...
    def test_kaggle_client_auth_overwrites_env(self, monkeypatch):
        """Latest provided token should be used for each check."""
        monkeypatch.setattr(kaggle_mcp, "KAGGLE_AVAILABLE", True)
        monkeypatch.setattr(kaggle_mcp.shutil, "which", lambda _: "kaggle")
        monkeypatch.delenv("KAGGLE_USERNAME", raising=False)
        monkeypatch.delenv("KAGGLE_API_KEY", raising=False)
        monkeypatch.delenv("KAGGLE_API_TOKEN", raising=False)

        kaggle_mcp.KaggleMCPClient(api_key="first-token")
        assert os.environ.get("KAGGLE_API_KEY") == "first-token"
        assert os.environ.get("KAGGLE_API_TOKEN") == "first-token"

        kaggle_mcp.KaggleMCPClient(api_key="second-token")
        assert os.environ.get("KAGGLE_API_KEY") == "second-token"
        assert os.environ.get("KAGGLE_API_TOKEN") == "second-token"
